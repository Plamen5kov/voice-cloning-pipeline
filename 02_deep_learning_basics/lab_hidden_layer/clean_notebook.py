...
NOTEBOOK_PATH = "audio_classification_nn.ipynb"
                ...
                if '# YOUR CODE STARTS HERE' in source_text and '# YOUR CODE ENDS HERE' in source_text:
                    ...
                    for line in lines:
                        if '# YOUR CODE STARTS HERE' in line:
                            ...
                            in_solution = True
                            # Capture indentation from the next line if it exists
                            continue
                    ...
                    # Convert back to list format with proper line breaks
                    cleaned_source = '\n'.join(new_lines)
                    cell['source'] = [line + '\n' for line in cleaned_source.split('\n')[:-1]]
                    if cleaned_source.split('\n')[-1]:  # Add last line without \n if it exists
                        cell['source'].append(cleaned_source.split('\n')[-1])
